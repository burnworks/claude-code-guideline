...
- 業務で Claude Code を使用する当社従業員
- 当社が開発業務を委託し、その業務内で Claude Code を使用する外部パートナー
...
Pro 以上のプランであれば、パーミッション、サンドボックス、Hooks 等、本ガイドラインで必要とするセキュリティ機能はすべて利用可能です。

| 対象 | 必要なプラン | 備考 |
|---|---|---|
| 当社従業員 | Max（当社から配布） | 当社が各従業員に Max プランのサブスクリプションを提供します |
...
```mermaid
flowchart TB
    L1["<b>第1層: パーミッション</b><br/>どのツール・ファイル・ドメインへのアクセスを許可するかを定義"]
    L2["<b>第2層: Hooks</b><br/>ツール実行の前後にカスタムチェックを挟み危険な操作を検知"]
    L3["<b>第3層: サンドボックス</b><br/>OS レベルでファイルシステムとネットワークを隔離"]
...
| 優先度 | ファイルの場所 | スコープ |
|---|---|---|
| 最高 | マネージド設定（管理者が OS レベルで配置）※ | 組織全体。ユーザー個人による上書き不可 |
...
| Anthropic アカウント | Pro 以上 | 当社従業員： Max（当社より配布）<br />外部パートナー： Pro 以上（第1章 1-3 参照） |
...
| `acceptEdits` | ファイル編集および `mkdir` / `touch` / `rm` / `rmdir` / `mv` / `cp` / `sed` を自動承認（ワーキングディレクトリ内のパスのみ）。その他のコマンドは確認 | コードレビュー体制が整っている場合のみ可 |
| `plan` | 計画の作成のみで、実行はしない | 探索・調査・設計検討時に有用 |
| `auto` | バックグラウンドの安全確認モデルが検証しながら自動実行（リサーチプレビュー）。**Max / Team / Enterprise / API プランのみ、Pro 不可** | 利用可（ただし研究用プレビュー機能のため慎重に） |
| `dontAsk` | `permissions.allow` で事前許可したツールとリードオンリーコマンドのみ実行。それ以外は自動的に拒否（確認プロンプトなし） | CI やスクリプトなど、実行ツールを事前定義できる環境で利用可 |
...
チーム全体で共有する設定は `CLAUDE.md` に記述してリポジトリにコミットしてください。一方、個人固有の設定（ローカル開発サーバーの URL、個人のテストデータ等）は同じディレクトリに `CLAUDE.local.md` を作成して記述し、`.gitignore` に追加することで Git 管理外に置くことができます。

### 6-2. 推奨テンプレート

`CLAUDE.md` はプロジェクトディレクトリで `/init` コマンドを実行することで自動生成できます。Claude Code がコードベースを分析し、ビルドコマンドやプロジェクトの規約を検出して初期ファイルを作成します。既に `CLAUDE.md` が存在する場合は上書きせず、改善案の提案のみ行います。

自動生成後は、以下のテンプレートを参考にしながらプロジェクトの実態に合わせて内容を調整してください。
...
業務での利用頻度が高いイベントを以下に示します。全イベントの一覧は[公式ドキュメント](https://code.claude.com/docs/en/hooks)を参照してください。

| イベント | 発火タイミング | 主な用途 |
|---|---|---|
| `PreToolUse` | ツール実行前 | 危険コマンドのブロック、入力の検証 |
| `PostToolUse` | ツール実行後（成功時） | ログ記録、結果の検証 |
| `PostToolUseFailure` | ツール実行後（失敗時） | エラーログの記録 |
| `UserPromptSubmit` | プロンプト送信後、Claude が処理する前 | 入力のフィルタリング・ログ |
| `Notification` | Claude Code が通知を送るタイミング | 外部通知連携（Slack 等） |
| `Stop` | Claude の応答完了時 | 追加作業の指示、検証の実行 |
| `SessionStart` | セッション開始・再開時 | セッションログの記録 |
| `SessionEnd` | セッション終了時 | セッションログの締め |
